...
abstract class TransformationTest extends ScalaLightCodeInsightFixtureTestAdapter {
  ...
  protected def check(@Language("Scala") before: String,
    ...
    implicit val headerAndFooter = (createHeader(header), createFooter(footer))

    val actualFile = configureByText(before)

    actualFile.depthFirst()
      .foreach(transform)
    assertEquals(after, slice(actualFile))
...
object TransformationTest {
  ...
  private def createFooter(footer: String) =
    filterEmptyLines(footer)
  ...
  private def slice(file: PsiFile)
    ...
    text.substring(header.length + 1, text.length - footer.length)
  }

  private def filterEmptyLines(string: String): String = {
    import java.lang.System.lineSeparator
    string.split(lineSeparator).toSeq
      .map(_.trim)
      .filterNot(_.isEmpty)
